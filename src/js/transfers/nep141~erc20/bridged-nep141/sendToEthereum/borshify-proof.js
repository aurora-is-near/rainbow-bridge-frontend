...
function borshifyOutcomeProof (proof) {
  const statusToBuffer = (status) => {
    ...
    if ('SuccessValue' in status) {
      ...
      return Buffer.concat([
        Buffer.from([2]),
        window.web3.utils.toBN(data.length).toBuffer('le', 4),
        data
      ])
  ...
  return Buffer.concat([
    window.web3.utils.toBN(proof.outcome_proof.proof.length).toBuffer('le', 4),
    Buffer.concat(
    ...
    Buffer.concat([
      ...
      window.web3.utils.toBN(proof.outcome_proof.outcome.gas_burnt).toBuffer('le', 8),
      window.web3.utils
      ...
      window.web3.utils.toBN(0).toBuffer('le', 4),
      ...
      window.web3.utils.toBN(proof.block_proof.length).toBuffer('le', 4),
      Buffer.concat(
